...
class HeartRateAdapter(private val mData: List<HeartRateData>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HeartRateAdapter.ViewHolder {
        return ViewHolder(
            LayoutInflater.from(parent.context).inflate(R.layout.item_hr, parent, false)
        )
    }
    ...
    inner class ViewHolder(v: View) : RecyclerView.ViewHolder(v) {
        ...
        fun bind(model: HeartRateData) {
            ...
            tvDate.text = WatchUtils.instance.clipTimeFormatSecond(model.heartStartTime)
            when (model.dataType) {
                "0" -> {
                    tVType.text = "手錶數值"
                }
