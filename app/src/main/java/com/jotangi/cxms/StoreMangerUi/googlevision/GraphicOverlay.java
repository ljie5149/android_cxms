...
public class GraphicOverlay extends View {
    ...
    private static final int EDGE_COLOR = Color.rgb(96, 157, 159);
    private static final float STROKE_WIDTH_DP = 4.0f;
    ...
    public GraphicOverlay(Context context, AttributeSet attrs) {
        ...
        edgePaint.setStrokeWidth(STROKE_WIDTH_DP * context.getResources().getDisplayMetrics().density);

        addOnLayoutChangeListener(
                (view, left, top, right, bottom, oldLeft, oldTop, oldRight, oldBottom) -> {
                    needUpdateTransformation = true;
                    lineLength = (right - left) / 8;
                }
        );
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        synchronized (lock) {
            ...
            int w = getWidth();
            int h = getHeight();
            canvas.drawLine(0, 0, lineLength, 0, edgePaint);
            canvas.drawLine(0, 0, 0, lineLength, edgePaint);
            canvas.drawLine(w - lineLength, 0, w, 0, edgePaint);
            canvas.drawLine(w, 0, w, lineLength, edgePaint);
            canvas.drawLine(0, h - lineLength, 0, h, edgePaint);
            canvas.drawLine(0, h, lineLength, h, edgePaint);
            canvas.drawLine(w - lineLength, h, w, h, edgePaint);
            canvas.drawLine(w, h - lineLength, w, h, edgePaint);
        }
    }
}
